...
(function (GameTestSebLi) {
    ...
    var GamePlayState = (function (_super) {
        ...
        GamePlayState.prototype.loadStaticImages = function () {
            ...
            brick = new GameTestSebLi.Brick(this.game, offsetX, offsetY, 'brick_1_1.png');
            var padding = brick.width + 2;
            var columns = 14;
            var rows = 4;
            if (GameTestSebLi.orientation == GameTestSebLi.gameOrientation.portrait) {
                rows = 8;
                columns = 7;
            }
            ...
            for (var y = 0; y < rows; ++y) {
                for (var x = 1; x < columns; ++x) {
                    brick = new GameTestSebLi.Brick(this.game, offsetX + (x * padding), offsetY + (y * rowPadding), 'brick_' + (y % 4 + 1) + '_1.png');
            ...
            //! load paddle
            this.paddle = new GameTestSebLi.Paddle(this.game, window.innerWidth / 2, window.innerHeight * 0.9);
        ...
        GamePlayState.prototype.cleanObjects = function () {
            this.ball.kill();
            this.paddle.kill();
            this.brickGroups.destroy(true);
        };
        GamePlayState.prototype.update = function () {
            if (window.innerWidth < window.innerHeight && GameTestSebLi.orientation != GameTestSebLi.gameOrientation.portrait) {
                GameTestSebLi.orientation = GameTestSebLi.gameOrientation.portrait;
                this.cleanObjects();
                this.loadStaticImages();
                this.game.scale.setGameSize(window.innerWidth, window.innerHeight);
                console.log("change to portrait");
            }
            else if (window.innerWidth > window.innerHeight && GameTestSebLi.orientation != GameTestSebLi.gameOrientation.lanscape) {
                GameTestSebLi.orientation = GameTestSebLi.gameOrientation.lanscape;
                this.cleanObjects();
                this.loadStaticImages();
                this.game.scale.setGameSize(window.innerWidth, window.innerHeight);
